...
function syncLearnAudioToggle(){const b=document.getElementById('learnAudioToggle');if(!b)return;const on=state.settings.autoSpeak!==false;b.textContent=on?'🔊 發音：開':'🔇 發音：關';b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on));b.setAttribute('aria-label',on?'自動發音已開啟':'自動發音已關閉')}
