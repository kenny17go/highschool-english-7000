...
function detailFor(w){const e=ENRICH[w.word]||{},a=WORD_DETAILS[w.word]||{};const syn=(e.syn&&e.syn.length)?e.syn:(a.syn||[]),ant=(e.ant&&e.ant.length)?e.ant:(a.ant||[]),conf=(e.conf&&e.conf.length)?e.conf:confusables(w);return {example:e.ex||genericExample(w),collocations:(e.col&&e.col.length)?e.col:genericCollocations(w),root:e.root||a.structure||rootHint(w.word),syn,ant,conf,family:a.family||[]}}
