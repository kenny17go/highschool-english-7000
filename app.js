...
const EXAMPLES_URL='data/vocabulary-examples.json?v=1.5.1';
let WORD_DETAILS={};
let WORD_EXAMPLES={};
...
function exitQuiz(){if(!document.getElementById('quizArea')||document.getElementById('quizArea').classList.contains('hidden'))return;const answered=Math.min(quizIndex+(currentQuestion?.userAnswer?1:0),quiz.length);if(!confirm(`確定要結束目前測驗嗎？\n\n已完成 ${answered} / ${quiz.length} 題。已作答紀錄會保留，未作答題目不會算錯。`))return;quiz=[];quizIndex=0;quizScore=0;currentQuestion=null;document.getElementById('quizArea').classList.add('hidden');document.getElementById('quizSetup').classList.remove('hidden');document.getElementById('examModeTabs').classList.remove('hidden');document.getElementById('examDesc').classList.remove('hidden');document.getElementById('quizFeedback').textContent='';document.getElementById('quizExplanationPanel')?.classList.add('hidden');document.getElementById('nextQuizBtn').classList.add('hidden');document.getElementById('spellingBox').classList.add('hidden');window.scrollTo({top:0,behavior:'smooth'})}
...
function renderOptions(opts,label,isCorrect,w){const el=document.getElementById('quizOptions');opts.forEach(o=>{const b=document.createElement('button');b.className='option';b.textContent=label(o);b.dataset.word=o.word;b.dataset.label=label(o);b.onclick=()=>answerQuiz(b,isCorrect(o),w,label(w),o);el.appendChild(b)})}
function answerQuiz(btn,correct,w,answerText,selected){document.querySelectorAll('.option').forEach(b=>b.disabled=true);state.stats.total++;currentQuestion.userAnswer=selected?.word||btn.dataset.word||btn.textContent;currentQuestion.correctAnswer=w.word;if(correct){btn.classList.add('correct');quizScore++;state.stats.correct++;document.getElementById('quizFeedback').textContent='答對了！'}else{btn.classList.add('wrong');state.mistakes[w.word]=(Number(state.mistakes[w.word])||0)+1;document.getElementById('quizFeedback').textContent=`正確答案：${answerText}`;document.querySelectorAll('.option').forEach(b=>{if(b.dataset.word===w.word)b.classList.add('correct')})}saveState();renderQuizExplanation();document.getElementById('nextQuizBtn').classList.remove('hidden')}
function submitSpelling(){if(!currentQuestion||currentQuestion.type!=='spelling')return;const {w}=currentQuestion,inp=document.getElementById('spellingInput'),ans=normalizeWord(inp.value),correct=ans===w.word;state.stats.total++;inp.disabled=true;currentQuestion.userAnswer=ans||'（未作答）';currentQuestion.correctAnswer=w.word;if(correct){quizScore++;state.stats.correct++;document.getElementById('quizFeedback').textContent='拼字正確！'}else{state.mistakes[w.word]=(Number(state.mistakes[w.word])||0)+1;document.getElementById('quizFeedback').textContent=`正確拼字：${w.word}`}saveState();renderQuizExplanation();document.getElementById('nextQuizBtn').classList.remove('hidden')}
...
function syncSettingsUI(){document.getElementById('dailyGoal').value=state.settings.dailyGoal||20;document.getElementById('studyMode').value=state.settings.mode||'7000';document.getElementById('autoSpeak').checked=state.settings.autoSpeak;syncLearnAudioToggle();const ez=document.getElementById('showExampleZh');if(ez)ez.checked=state.settings.showExampleZh!==false!==false}
