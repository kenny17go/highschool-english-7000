...
def main():
    ...
    EXAMPLES.write_text(json.dumps({'version':VERSION,'generatedAt':now(),'policy':{'fakeFallbackDisabled':True,'preferredLength':'8-22 words','priority':'GSAT corpus > recent years > Level 1-6 > Level 7','externalTextImport':'disabled in V1.4.8; metadata only'},'summary':{'words':len(rows),'ready':sum(r['status']=='ready' for r in rows.values()),'pending':sum(r['status']!='ready' for r in rows.values()),'qualityGrades':grade_counts,'sources':source_counts},'words':rows},ensure_ascii=False,separators=(',',':')),encoding='utf-8')
    SOURCES.write_text(json.dumps({'version':VERSION,'generatedAt':now(),'sources':{'site_reviewed':{'name':'高中英文7000本站審核例句','type':'original/reviewed','license':'site-authored','textImported':True},'princeton_wordnet':{'name':'Princeton WordNet via NLTK','type':'candidate metadata','license':'WordNet License','textImported':False},'words_tw_reference':{'name':'words.tw','type':'reference only','textImported':False,'note':'僅參考覆蓋率與呈現方式，不複製例句'},'tatoeba':{'name':'Tatoeba','type':'future candidate source','license':'CC BY 2.0 FR / sentence-specific','textImported':False}}},'rules':{'attributionRequired':True,'externalExampleTextImported':False}},ensure_ascii=False,separators=(',',':')),encoding='utf-8')
