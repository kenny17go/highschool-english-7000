...
def main():
    ...
    AUDIT.write_text(json.dumps({'version':VERSION,'generatedAt':now(),'summary':{'words':len(rows),'ready':ready,'pending':pending,'aiOriginalReady':ai_ready,'preservedReady':preserved_ready,'coverage':round(ready/max(1,len(rows)),6)},'issues':issues},ensure_ascii=False,separators=(',',':')),encoding='utf-8')
